...
## Partiel
```bash
clear && sudo bash -c 'echo "===== SYSTEM ====="; echo ""; uname -a; hostname; cat /etc/os-release; uname -m; uname -r; grep -m1 "model name" /proc/cpuinfo; free -h; head -5 /proc/meminfo; echo ""; echo "===== DISKS ====="; lsblk -f -e7; lsblk -e7 -o NAME,FSTYPE,MOUNTPOINT,SIZE,UUID,LABEL,PARTUUID; grep -vE "loop[0-9]+$" /proc/partitions; fdisk -l 2>/dev/null; parted -l 2>/dev/null; blkid | grep -v "^/dev/loop"; df -hT; findmnt --fstab --raw; pvs 2>/dev/null; vgs 2>/dev/null; lvs 2>/dev/null; echo ""; echo "===== BOOT ====="; cat /proc/cmdline; efibootmgr -v 2>/dev/null; findmnt -n -o SOURCE,TARGET,FSTYPE,OPTIONS /boot/efi; ls -la /boot/efi/EFI 2>/dev/null; for d in $(lsblk -dn -e7,11 -o NAME); do dd if=/dev/$d bs=512 count=1 2>/dev/null | strings | grep -iE "grub|ntldr|bootmgr|lilo"; done; ls /boot | grep vmlinuz; command -v os-prober >/dev/null 2>&1 && os-prober 2>/dev/null || echo "os-prober non installe"; echo ""; echo "===== GRUB ====="; cat /etc/default/grub 2>/dev/null; ls -l /etc/grub.d 2>/dev/null; GRUBCFG=""; for p in /boot/grub/grub.cfg /boot/grub2/grub.cfg /boot/efi/EFI/*/grub.cfg; do [ -f "$p" ] && GRUBCFG="$p" && break; done; if [ -n "$GRUBCFG" ]; then echo "grub.cfg: $GRUBCFG"; cat "$GRUBCFG"; else echo "grub.cfg introuvable"; fi; grub-install --version 2>/dev/null; echo ""; echo "===== SECUREBOOT ====="; mokutil --sb-state 2>/dev/null; dmesg | grep -iE "secureboot|efi" | tail -20; echo ""; echo "===== TPM ====="; ls /sys/class/tpm/ 2>/dev/null; dmesg | grep -i tpm | tail -20; echo ""; echo "===== LUKS ====="; lsblk -f | grep crypto_LUKS; cat /etc/crypttab 2>/dev/null; echo ""; echo "===== RAID ====="; cat /proc/mdstat; echo ""; echo "===== FILESYSTEM ====="; cat /etc/fstab; swapon --show; echo ""; echo "===== HARDWARE ====="; lspci -nnk | grep -A3 -iE "storage|vga|3d"; echo ""; echo "===== LOGS ====="; journalctl -p 3 -xb --no-pager -n 50; dmesg | tail -50' 2>&1 | tee >(nc termbin.com 9999)

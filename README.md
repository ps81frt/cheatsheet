...
## FULL
```bash
clear && sudo bash -c 'echo "===== SYSTEM ====="; uname -a; hostname; cat /etc/os-release; uname -m; uname -r; grep -m1 "model name" /proc/cpuinfo; free -h; head -5 /proc/meminfo; echo ""; echo "===== DISKS ====="; lsblk -f; lsblk -o NAME,FSTYPE,MOUNTPOINT,SIZE,UUID,LABEL,PARTUUID; cat /proc/partitions; fdisk -l 2>/dev/null; parted -l 2>/dev/null; blkid; df -hT; findmnt --fstab --raw; echo ""; echo "===== LVM ====="; pvs 2>/dev/null; vgs 2>/dev/null; lvs 2>/dev/null; echo ""; echo "===== BOOT SECTOR SIGNATURES ====="; for d in $(lsblk -dno NAME -e7,11); do echo "--- /dev/$d ---"; dd if=/dev/$d bs=512 count=1 2>/dev/null | strings | grep -iE "grub|ntldr|bootmgr|lilo"; done; echo ""; echo "===== BOOT ====="; cat /proc/cmdline; efibootmgr -v 2>/dev/null; findmnt -n -o SOURCE,TARGET,FSTYPE,OPTIONS /boot/efi; ls -la /boot/efi/EFI 2>/dev/null; ls /boot; echo ""; echo "===== GRUB ====="; cat /etc/default/grub; ls -l /etc/grub.d; echo "--- grub.cfg reel ---"; cat /boot/grub/grub.cfg 2>/dev/null; echo ""; echo "===== OS-PROBER ====="; which os-prober && os-prober; echo ""; echo "===== SECUREBOOT ====="; mokutil --sb-state 2>/dev/null; dmesg | grep -iE "secureboot|efi" | tail -20; echo ""; echo "===== TPM ====="; ls /sys/class/tpm/ 2>/dev/null; dmesg | grep -i tpm | tail -20; echo ""; echo "===== LUKS ====="; lsblk -f | grep crypto_LUKS; cat /etc/crypttab 2>/dev/null; echo ""; echo "===== RAID ====="; cat /proc/mdstat; echo ""; echo "===== FILESYSTEM ====="; cat /etc/fstab; swapon --show; lsblk -o NAME,FSTYPE,MOUNTPOINT,SIZE,UUID; echo ""; echo "===== HARDWARE ====="; lspci -nnk | grep -A3 -iE "storage|vga|3d"; echo ""; echo "===== LOGS ====="; journalctl -p 3 -xb --no-pager -n 50; dmesg | tail -50' 2>&1 | nc termbin.com 9999
```
## Partiel
```bash
clear && sudo bash -c 'echo "===== SYSTEM ====="; echo; echo "--- uname -a ---"; uname -a; echo "--- hostname ---"; hostname; echo "--- os-release ---"; cat /etc/os-release; echo "--- arch ---"; uname -m; echo "--- kernel ---"; uname -r; echo "--- cpu ---"; grep -m1 "model name" /proc/cpuinfo; echo "--- RAM ---"; free -h; head -5 /proc/meminfo; echo; echo "===== DISKS ====="; echo; echo "--- lsblk -f ---"; lsblk -f -e7; echo; echo "--- lsblk detail ---"; lsblk -e7 -o NAME,FSTYPE,MOUNTPOINT,SIZE,UUID,LABEL,PARTUUID; echo; echo "--- partitions ---"; grep -vE "loop[0-9]+$" /proc/partitions; echo; echo "--- fdisk ---"; fdisk -l 2>/dev/null; echo; echo "--- parted ---"; parted -l 2>/dev/null; echo; echo "--- blkid ---"; blkid | grep -v "^/dev/loop"; echo; echo "--- df ---"; df -hT; echo; echo "--- fstab mounts ---"; findmnt --fstab --raw; echo; echo "--- LVM ---"; pvs 2>/dev/null; vgs 2>/dev/null; lvs 2>/dev/null; echo; echo "===== BOOT ====="; echo; echo "--- cmdline ---"; cat /proc/cmdline; echo; echo "--- EFI boot ---"; efibootmgr -v 2>/dev/null; echo; echo "--- EFI mount ---"; findmnt -n -o SOURCE,TARGET,FSTYPE,OPTIONS /boot/efi; echo; echo "--- EFI files ---"; ls -la /boot/efi/EFI 2>/dev/null; echo; echo "--- boot sectors ---"; for d in $(lsblk -dn -e7,11 -o NAME); do echo "> /dev/$d"; dd if=/dev/$d bs=512 count=1 2>/dev/null | strings | grep -iE "grub|ntldr|bootmgr|lilo"; done; echo; echo "--- kernels ---"; ls /boot | grep vmlinuz; echo; echo "--- os-prober ---"; command -v os-prober >/dev/null && os-prober 2>/dev/null || echo "not installed"; echo; echo "===== GRUB ====="; echo; echo "--- grub default ---"; cat /etc/default/grub 2>/dev/null; echo; echo "--- grub.d ---"; ls -l /etc/grub.d 2>/dev/null; echo; echo "--- grub.cfg ---"; GRUBCFG=""; for p in /boot/grub/grub.cfg /boot/grub2/grub.cfg /boot/efi/EFI/*/grub.cfg; do [ -f "$p" ] && GRUBCFG="$p" && break; done; if [ -n "$GRUBCFG" ]; then echo "found: $GRUBCFG"; cat "$GRUBCFG"; else echo "missing grub.cfg"; fi; echo; echo "--- grub version ---"; grub-install --version 2>/dev/null; echo; echo "===== SECUREBOOT ====="; echo; mokutil --sb-state 2>/dev/null; dmesg | grep -iE "secureboot|efi" | tail -20; echo; echo "===== TPM ====="; ls /sys/class/tpm/ 2>/dev/null; dmesg | grep -i tpm | tail -20; echo; echo "===== LUKS ====="; lsblk -f | grep crypto_LUKS; cat /etc/crypttab 2>/dev/null; echo; echo "===== RAID ====="; cat /proc/mdstat; echo; echo "===== FILESYSTEM ====="; cat /etc/fstab; swapon --show; echo; echo "===== HARDWARE ====="; lspci -nnk | grep -A3 -iE "storage|vga|3d"; echo; echo "===== LOGS ====="; journalctl -p 3 -xb --no-pager -n 50; dmesg | tail -50' 2>&1 | tee >(nc termbin.com 9999)
